...
def format_schema_for_display(schema):
    ...
    for table_name, columns_list in schema.items():
        ...
        # Get datetime column info for this table
        table_datetime_info = datetime_info.get(table_name, [])
        datetime_col_names = [col['column'] for col in table_datetime_info] if table_datetime_info else []
        
        for name, dtype in columns_list:
            if name in datetime_col_names:
                # Highlight datetime columns
                schema_text += f"  - {name} ({dtype}) ⏰ DATETIME COLUMN\n"
            else:
...
def render_sqlbot(llm=None):
    ...
    if schema:
        ...
        # Add quick examples including datetime examples
...
def process_sql_query(nl_query, schema_text, llm, conn, is_generate_only, is_advanced_mode):
    ...
    try:
        with st.spinner(f"🤖 Converting natural language to SQL..."):
            # Generate SQL using enhanced LLM
            sql_query_generated = enhanced_generate_sql_from_nl(nl_query, schema_text, llm, is_advanced_mode)
            ...
            if not is_generate_only:
                enhanced_execute_sql_query(sql_query_generated, conn)
