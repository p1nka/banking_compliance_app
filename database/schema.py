...
def enhanced_get_schema_with_datetime_info(conn):
    ...
    This is the MAIN enhancement for datetime handling.
    """
    try:
...
def diagnose_datetime_issues():
    ...
    Run this to troubleshoot datetime problems.
    """
    ...
    try:
        ...
        datetime_test_queries = [
            ...
            ("Date range",
             "SELECT COUNT(*) FROM accounts_data WHERE Account_Creation_Date BETWEEN '2020-01-01' AND '2024-01-01'"),
            ("Last 30 days",
             "SELECT COUNT(*) FROM accounts_data WHERE Account_Creation_Date >= DATEADD(DAY, -30, GETDATE())"),
        ]
...
def get_db_schema():
    ...
    This function is called by the SQL Bot to get the database schema.
    """
    try:
        conn = get_db_connection()
        if conn is None:
        ...
        # Try enhanced schema detection first (NEW ENHANCEMENT)
        try:
            enhanced_schema = enhanced_get_schema_with_datetime_info(conn)
            if enhanced_schema:
...
def insert_sample_data_if_empty():
    ...
    try:
        ...
        if count == 0:
            ...
            sample_data = [
                ('ACC001', 'CUST001', 'Savings', 'AED', '2020-01-01', 5000.00, '2021-01-01', '2021-01-01', 'Yes', 'No',
                 'No'),
                ('ACC002', 'CUST002', 'Current', 'AED', '2021-01-01', 15000.00, '2024-01-01', '2024-01-01', 'No', 'Yes',
                 'Yes'),
                (
                'ACC003', 'CUST003', 'Savings', 'USD', '2019-06-15', 25000.00, '2020-12-01', '2020-12-01', 'Yes', 'Yes',
                'No'),
            ]
...
# Additional utility functions for datetime handling
def get_datetime_columns_info():
    """
    Get information about datetime columns from session state.
    This is used by other modules to understand datetime column types.
    """
...
def get_datetime_format_for_column(table_name, column_name):
    """
    Get the appropriate datetime format for a specific column.
    Returns format string for use in SQL queries.
    """
    is_datetime, dtype = is_datetime_column_in_table(table_name, column_name)
    if is_datetime:
        if dtype == 'date':
            return 'YYYY-MM-DD'
...
def clear_schema_cache():
    """
    Clear the cached schema data. Use this when schema changes.
    """
    if hasattr(st.cache_data, 'clear'):
        st.cache_data.clear()
    # Also clear datetime info from session state
